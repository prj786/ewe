...
- The bar runs edge to edge along the top: the workspace chip and the focused app on the left, then tray, updates, keyboard layout, and the Quick settings button (status indicators and the clock in one pill) on the right. The dock floats at the bottom of the main screen with the launchers and one group per workspace. Keep that placement; restyle, don't rearrange.
- The bar is `bar-height` (40px) by default, or `bar-height-lg` (48px) for a roomier layout. Size bar modules and their icons to match: `control-lg` with `icon-lg` in the 40px bar, `control-xl` with `icon-xl` in the 48px bar.
