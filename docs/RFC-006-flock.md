...
first-party Tauri app in its own repo `prj786/ewe-sync`, shipped preinstalled
...
writes the manifest; ewe-settings edits `ewe.conf`; **ewe-sync** moves the one
file and your folders between machines. Nothing else has a sync button.

Name: **ewe-sync** — it sits next to `ewe-auth`, `ewe-drive`, `ewe-cloud`
and `ewe-conf`, and the family keeps one naming (owner decision,
2026-09-02; drafted as "Flock" for an afternoon). Binary `ewe-sync`, desktop
id `io.github.prj786.ewe-sync`.
...
already implements it; ewe-sync drives that, it does not reimplement it.
...
provider may or may not enable. So ewe-sync cannot register users on Murena,
...
`--path`, `--exclude`). ewe-sync's folder sync = *our* UI and scheduler
...
over WebDAV with `If-Match`, the manifest read by Komble. ewe-sync adds a
...
local path is per-machine and lives in `~/.local/state/ewe/ewe-sync.json`):
...
| # | what | gate |
|---|---|---|
| F1 | repo `prj786/ewe-sync`: Tauri scaffold in the family design system, tray, Account + This machine + Machines panes on top of ewe-cloud/ewe-conf; PKGBUILD; CI (fmt, clippy, build, release on tag) | builds; signs in against the mock; `ewe.conf` round-trips through the UI |
...
| F3 | integration: ewe package depends on ewe-sync; Welcome/Settings launchers; ewe-repo publish builds it from its release; website page | ISO 0.9-alpha boots to Welcome → sign in → ewe-sync tray appears signed in |
| F4 | QEMU e2e on the owner's account, then bare metal | the restore demo, recorded |

Order with RFC-005: phases 1–4 of RFC-005 are the plumbing ewe-sync sits on and
...
1. ~~Name~~ — decided: **ewe-sync**, the family naming (was Flock).
...
4. Does ewe-sync also own the Google optional card (mail/Drive), or does that
