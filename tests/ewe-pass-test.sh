...
m="$(bin/ewe-pass match --class discord --title 'Discord' --limit 3)"
...
m="$(bin/ewe-pass match --class kitty --title 'shell')"
...
m="$(bin/ewe-pass match --class zen --title 'Sign in to GitHub — Zen')"
...
m="$(bin/ewe-pass match --class nothing --title 'nothing')"
...
m="$(bin/ewe-pass match --class nothing --title x --query rout)"
check "--query filters on title" "echo '$m' | python3 -c 'import json,sys; d=json.load(sys.stdin); assert [i[\"id\"] for i in d[\"items\"]]==[\"ccc\"], d'"
bin/ewe-pass remember ccc --class Nothing >/dev/null
m="$(bin/ewe-pass match --class nothing --title x)"
...
bin/ewe-pass forget ccc --class nothing >/dev/null
...
bin/ewe-pass match --refresh --class x --title y >/dev/null
...
r="$(FAKE_PID=4242 bin/ewe-pass fill aaa --pid 4242)"
...
r="$(bin/ewe-pass copy aaa --what username)"
...
m="$(bin/ewe-pass match --class a --title b || true)"
