...
**Sandboxed by default.** `up` runs the shell with `HOME` and every `XDG_*`
dir under `/tmp/hs-driver/home`: a fresh `ewe.conf`, a `theme-tokens.json`
generated by this checkout's `bin/ewe-theme`, and this checkout's
fontconfig (so the shipped Geist fonts resolve). The live `~/.config` is
